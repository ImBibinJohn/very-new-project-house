...
from projecthouse.settings import EMAIL_HOST_USER, MEDIA_ROOT
...
def myprofile(request):
    if 'admin' in request.session:
        pro = profile.objects.all()
        return render(request, 'Administrator/my profile.html',
                      {'profile': pro})
    else:
        return redirect('admin_login')
...
def userdashboard(request):
    if request.method == 'POST':
        ...
        data = []
        if searchQuery != '':
            searchResult = Addnewproject.objects.filter(
                projectname__icontains=searchQuery).values()
            print(searchResult)

            searchResultant = Addnewproject.objects.filter(
                projectname__icontains=searchQuery)
...
def addproject(request):
    ...
    context = {'projectname': projectname,
               'projectname_count': projectname_count, 'platformname': platformname}
...
def usermlprojects(request):
    if request.method != 'POST':
        project = Addnewproject.objects.filter(
            selectplatform='Machine Learning')
        context = {'projects': project}
...
def addnewplatformdb(request):
    if request.method == 'POST':
        ...
        plat = Addnewplatform(platformname=platformname,
                              uploadthumbnail=uploadthumbnail1, description=description)
        plat.save()
        return redirect('adminDash')
    else:
        return render(request, 'Administrator/dashboard.html')
...
def search(request):
    if request.method == 'POST':
        ...
        pro = Q(projectname__icontains=usr) | Q(
            languageused__icontains=usr) | Q(shortdescription__icontains=usr)
...
def result(request):
    if 'q' in request.GET and request.GET['q']:
        ...
        context = {'name': value}
        return render(request, 'Administrator/requested projects.html', context)
    else:
        return page_not_found(request)
...
def userreqprojects(request):
    req1 = Userrequestproject.objects.all()
    context = {'req': req1}
    return render(request, 'Administrator/user request projects.html', context)
...
def getproject(request, id):
    if request.method == 'POST':
        ...
        phone = request.POST['phone']
        project = Addnewproject.objects.get(id=id)
        getpro = GetProject.objects.create(
            name=name, email=email, phone=phone, project_Detail=project)
        getpro.save()
...
def add_ieee(request):
    ...
    return render(request, 'Administrator/add new ieee.html', context)
...
def add_ieee_db(request):
    if request.method == "POST":
        ...
        selectplatform = request.POST['selectplatform']
        uploadpdf = request.FILES.get('uploadpdf')
        paper = add_new_ieee.objects.create(
            title=title, selectplatform=selectplatform, uploadpdf=uploadpdf)
        paper.save()
        return redirect('adminDash')
    else:
...
def activate(request):
    ...
    if request.method == "POST":
        ...
        if random_otp == otp_verify:
            ...
            messages.success(
                request, 'The submitted code has been verified. We will be contacting you soon.')
            return render(request, 'Administrator/activate.html')
        else:
            messages.error(
                request, 'Invalid code submitted. Please try again.')
            return render(request, 'Administrator/activate.html')
    else:
        return render(request, 'Administrator/activate.html')
...
def savemockq(request):
    if request.method == 'POST':
        ...
        inte = QuesModel.objects.create(
            platform_name=plat, question=q1, op1=a1, op2=a2, op3=a3, op4=a4, ans=aa)
        inte.save()
...
def takemocktest(request):
    if request.method == 'POST':
        ...
        items = list(QuesModel.objects.filter(
            platform_name=plat).order_by('id')[:10])
...
def uploadtutorial(request):
    if request.method == 'POST':
        ...
        title = request.POST['title']
        uploadvideo = request.FILES.get('uploadvideo')
        getpro = tutorial.objects.create(
            platform_name=plat, videotitle=title, uploadvideo=uploadvideo)
        getpro.save()
        platformname = Addnewplatform.objects.all()
        tuto = tutorial.objects.all()
        return render(request, 'Administrator/adminaddvediotutorials.html', {'plat': platformname, 'tutorial': tuto})
    ...
    try:
        if request.method == 'POST':
            ...
            if Course.objects.filter(name=name, level=level):
                ...
                cour = Course(name=name, shortdescription=shortdescription,
                              description=description, modules=modules, level=level, platformid=plat)
                cour.save()
    ...
    try:
        if request.method == 'POST':
            ...
            if Lecture.objects.filter(name=name):
                messages.info(
                    request, 'Lecture with same name already exists.')
                return redirect('addlecturepage')
            else:
                lect = Lecture(name=name, description=description,
                               video=video, courseid=cour)
                lect.save()
    ...
    try:
        if request.method == 'POST':
            ...
            if Qanda.objects.filter(question=question):
                ...
                quest = Qanda(question=question, correct=correct, optiona=optiona,
                              optionb=optionb, optionc=optionc, lectureid=lect, courseid=cour)
                quest.save()
...
def test(request):
    try:
        q = list(Qanda.objects.filter(
            courseid=request.session['level']).order_by('?')[:10])
        random.shuffle(q)
        context = {'q': q}
...
def userandroid(request):  # userprojects
    project = Addnewproject.objects.filter(selectplatform='Android')
    context = {'projects': project}
    return render(request, 'user/user android projects.html', context)
...
def usermlprojects(request):
    if request.method != 'POST':
        project = Addnewproject.objects.filter(
            selectplatform='Machine Learning')
        context = {'projects': project}
        return render(request, 'user/user ml projects.html', context)
    else:
        return render(request, 'user/user_dasboard.html')
...
def user_req_inbuilt_projectss(request, id):
    requested_paper = Addnewproject.objects.get(id=id)
    context = {'requested_paper': requested_paper}
    return render(request, 'user/user_request_inbuilt_projects.html', context)
...
def user_req_inbuilt_projectdb(request):
    if request.method == 'POST':
        ...
        req2.save()
        messages.success(request, 'Thank you, Admin will get to you soon')
    return render(request, 'user/user_request_inbuilt_projects.html')
...
def adminieeeviews(request, id):
    ...
    if request.method != 'POST':
        plat = Addnewplatform.objects.all()
        project = add_new_ieee.objects.filter(selectplatform=mem)

        return render(request, 'Administrator/view_ieee_papers.html', {'plat': plat, 'project': project})
    else:
        return render(request, 'Administrator/dashboard.html')
